...
class PersonasController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        $model = $this->findModel($id);
        //eliminar anteriores
        foreach ($model->relPersonaTareas as $rel) {
            $rel->delete();
        }
        if ($model->load(Yii::$app->request->post())) {
            foreach (Yii::$app->request->post()['Personas']['relPersonaTareas'] as $i => $id_tarea) {
                $relPersonaTareas = new \app\models\RelPersonaTarea();
                $relPersonaTareas->id_tarea = $id_tarea;
                $relPersonaTareas->id_persona = $model->id;
                $relPersonaTareas->save();
            }
            //$model->relPersonaTareas=$rels;
            $model->save();
            //echo "<pre>";
            //print_r(Yii::$app->request->post()['Personas']['relPersonaTareas']);
            //echo "</pre>";
            return $this->redirect(['view', 'id' => $model->id]);
        } else {
